		...
		try {
			...
			pstmt = conn.prepareStatement(
					"select rest.idval idval, rest.restaurant_name res_name, rest.store_address addr, rest.office_hours offhour, \r\n"
							...
							+ "nvl(menu_list.list,'none') as list,"
							+ "reim.list ima_list, rest.image_location imgloc\r\n" + "from restaurant rest\r\n"
							+ "left join (select mm.idval, LISTAGG(mm.food_name,',') WITHIN GROUP (ORDER BY mm.idval ) AS list \r\n"
							+ "from main_menu mm group by idval) menu_list\r\n" + "on rest.idval = menu_list.idval \r\n"
							+ "left join (select re.idval, avg(re.grade) grade from review re group by idval) rev\r\n"
							+ "on rest.idval = rev.idval\r\n"
							+ "left join (select ri.idval, LISTAGG(ri.image_location,',') WITHIN GROUP (ORDER BY ri.idval ) AS list \r\n"
							+ "from restaurant_image ri group by idval) reim\r\n" + "on rest.idval = reim.idval\r\n"
							+ "where rest.idval = ?");
			pstmt.setInt(1, t);
